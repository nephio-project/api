...
func TestConditionEqual(t *testing.T) {
	cases := map[string]struct {
		...
		"DifferentType": {
			...
			b:    Condition{metav1.Condition{Type: string("bla")}},
			want: false,
		},
		"DifferentStatus": {
		...
		"DifferentReason": {
			...
			b:    Condition{metav1.Condition{Reason: "other"}},
			want: false,
		},
		"DifferentMessage": {
...
func TestConditionedStatusEqual(t *testing.T) {
	cases := map[string]struct {
		...
		"Identical": {
			a:    NewConditionedStatus(Ready(), Ready()),
			b:    NewConditionedStatus(Ready(), Ready()),
			want: true,
		},
		"IdenticalDifferentOrder": {
			a:    NewConditionedStatus(Condition{Condition: metav1.Condition{Type: "a"}}, Ready()),
			b:    NewConditionedStatus(Ready(), Condition{Condition: metav1.Condition{Type: "a"}}),
			want: true,
		},
		"DifferentAmount": {
			a:    NewConditionedStatus(Unknown(), Ready()),
			b:    NewConditionedStatus(Condition{Condition: metav1.Condition{Type: "a"}}),
			want: false,
		},
		"DifferentCondition": {
			a:    NewConditionedStatus(Ready(), Ready()),
			b:    NewConditionedStatus(Ready(), Failed(errors.New("a").Error())),
			want: false,
		},
		"Anil": {
			a:    nil,
...
func TestSetConditions(t *testing.T) {
	cases := map[string]struct {
		...
		"TypeDoesNotExist": {
			cs:   NewConditionedStatus(Condition{Condition: metav1.Condition{Type: "A"}}),
			c:    []Condition{Ready()},
			want: NewConditionedStatus(Condition{Condition: metav1.Condition{Type: "A"}}, Ready()),
		},
	}
